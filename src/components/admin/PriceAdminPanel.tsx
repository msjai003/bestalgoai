...
import { PriceAdminPlan } from '@/hooks/usePriceAdmin';

const PriceAdminPanel = () => {
  const { toast } = useToast();
  const { user } = useAuth();
  const [plans, setPlans] = useState<PriceAdminPlan[]>([]);
  ...
  const handlePlanChange = (planId: string, field: keyof PriceAdminPlan, value: any) => {
  ...
  if (!user || !isAdmin) {
    return (
      <div className="bg-gray-900 text-white p-6 rounded-lg">
        <p>You need administrator permissions to access the pricing admin panel.</p>
      </div>
    );
  }
