...
interface Question {
  id: string | number;  // Accept both string and number for compatibility
  question: string;
  answer: string;
  display_order: number;
}

const IntermediateQuestionsSection = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        setLoading(true);
        const result = await fetchQuestionsByLevel('intermediate');
        
        if (result && result.questions.length > 0) {
          setQuestions(result.questions);
        } else {
          setError('No intermediate questions available at the moment.');
        }
      } catch (err) {
        console.error('Error fetching intermediate questions:', err);
        setError('Failed to load intermediate questions. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading && activeTab === "flashcards") {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-4 text-gray-400">Loading intermediate questions...</p>
      </div>
    );
  }

  if (error && activeTab === "flashcards") {
    return (
      <div className="text-center py-12">
  ...
  if (questions.length === 0 && activeTab === "flashcards") {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400">No intermediate questions available at the moment.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-6">
      <h2 className="text-xl font-bold mb-4">Intermediate Trading</h2>
                ...
                <div className="flex flex-col justify-between h-full">
                  <div>
                    <div className="text-xs text-gray-400 mb-1">
                      Intermediate • Question {currentQuestionIndex + 1}
                    </div>
...
export default IntermediateQuestionsSection;
